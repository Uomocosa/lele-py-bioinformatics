...
def main_cuda():
    # pixi run -e cuda python -c "from bio.mlp_train_featurize_k5_fold_only_fingerprints import main_cuda; main_cuda()"
...
def test_():
    # pixi run -e cpu python -c "from bio.mlp_train_featurize_k5_fold_only_fingerprints import test_; test_()"
    # lele.Loguru.simple_format()
    logger.remove()
    logger.add(sys.stderr, level="INFO")
    dataset_config = bio.Dataset.PDCC.Config(
