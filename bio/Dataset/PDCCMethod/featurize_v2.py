...
def featurize_v2(
    ...
    molecule_features = bio.Metric.calculate_logd(
        molecule_features, "DRUG", 
        options.ph_min, options.ph_max, options.precision,
...
def test_():
    ...
    df = featurize_v2(df)
    print(df.head(10))
    df.to_csv(HELPER_DIR / "featurize_v2.csv", index=False)
