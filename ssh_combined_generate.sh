...
echo "--- RUNNING: 'combined_generate' ---"
# CHECKPOINTS_DIR="lele/MinGPT/__HELPER_DIR__/checkpoints"
CHECKPOINTS_DIR="COMBINED_checkpoints"
# TO TEST USE THIS: 
# uv run --no-sync combined_generate --model_dir="$CHECKPOINTS_DIR"_test
# OTHERWISE USE THIS:
uv run --no-sync combined_generate --model_dir="$CHECKPOINTS_DIR" --batch_size=128 --smiles_to_generate=12800
